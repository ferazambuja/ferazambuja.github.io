...
(() => {
  ...
  function render(){
    ...
    $('atlas-condition').textContent=[record.label,experiment.condition,`${rows.length} individual readings shown`].filter(Boolean).join(' · ');
    ...
    $('atlas-mean-rows').replaceChildren(...record.means.map(m=>{const tr=node('tr');tr.append(node('td',friendly(m.pattern)),node('td',triplet(m.xyz)),node('td',triplet(m.lab)));return tr;}));
    const series=rows.map(r=>({...r,color:color(r),name:friendly(r.id),highlight:r.id===selected}));
    if(!pattern)record.means.forEach(m=>series.push({...m,color:color(m),name:m.pattern,mean:true}));
    const highlighted=rows.find(r=>r.id===selected);
    if(highlighted)highlighted.evaluations.filter(e=>e.predicted_spectrum).forEach((e,i)=>series.push({spectrum:e.predicted_spectrum,color:colors[(i+2)%colors.length],name:e.label,dashed:true,mean:true}));
    ...
    $('atlas-series-key').replaceChildren(...(record.means.length&&!pattern?record.means:patterns.filter(p=>!pattern||p===pattern).map(p=>record.readings.find(r=>r.pattern===p))).map(m=>{const s=node('span');const key=node('i');key.style.color=color(m);s.append(key,document.createTextNode(friendly(m.pattern)));return s;}));
    ...
    $('atlas-readings').replaceChildren(...rows.map((r,i)=>{const tr=node('tr');tr.dataset.selected=r.id===selected;const cell=node('td'),b=node('button',`${i+1} · ${friendly(r.id)}`);b.addEventListener('click',()=>{selected=selected===r.id?null:r.id;render();});cell.append(b);tr.append(cell,node('td',friendly(r.pattern)+(r.context_role?' · '+r.context_role:'')),node('td',triplet(r.xyz)),node('td',triplet(r.lab)),node('td',r.temperature_c==null?'—':`${number(r.temperature_c)} °C`));return tr;}));
    ...
    $('atlas-errors').replaceChildren(...evaluations.map(({r,e})=>{const tr=node('tr');tr.append(node('td',friendly(r.id)),node('td',`${e.label} (${e.kind==='target'?'desired color':e.kind==='reference'?'reference':e.kind==='context'?'context forecast':e.kind==='development'?'development':'output forecast'})`),node('td',triplet(e.predicted_xyz)),node('td',number(e.delta_e)),node('td',number(e.limit)),node('td',`${e.direction}: ${triplet(e.residual_xyz)}`),node('td',e.status==='PASS'?'Within chosen limit':e.status==='FAIL'?'Outside chosen limit':e.status||'—'));return tr;}));
  }
  function chooseRecord(id,pattern){record=experiment.records.find(r=>r.id===id)||experiment.records[0];$('atlas-pattern').value=record.id;options('atlas-reading-pattern',[['','All patterns'],...[...new Set(record.readings.map(r=>r.pattern))].map(p=>[p,friendly(p)])],pattern||'');selected=null;render();}
  function chooseExperiment(id,comparison,pattern){experiment=data.experiments.find(e=>e.id===id);$('atlas-experiment').value=id;options('atlas-pattern',experiment.records.map(r=>[r.id,r.label]),comparison);chooseRecord($('atlas-pattern').value,pattern);}
